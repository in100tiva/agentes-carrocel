...
1. Avaliar **cada imagem** contra pipeline/data/design-system-carousel-instagram.md (CAPA/CONTEÚDO/FECHAMENTO, hierarquia em 4 níveis, camadas z-index, número atrás do título, seta orgânica, **um** box por slide, dots bottom-center) e pipeline/data/brand-mvp-flow-colors.md (paleta, tipografia, riscos/círculos em palavras-chave). Incluir **[`carousel-layout-rules.md`](../pipeline/data/carousel-layout-rules.md):** slides internos com título/corpo centralizados (exceto caixas); **se N ≥ 6 slides:** imagens **02, 04 e 06** devem mostrar **gráfico de barras horizontais** (SVG) — se ausente ou ilegível → **REJECT** com `slides_to_redo` e `correction_brief` citando §10; slides 2/4/6 sem legendas de valor do gráfico **sobre** a barra (gutter à direita no SVG). **Continuidade panorâmica:** verificar que `.bg-mascot` usa `left: 0; right: 0` (sem expansão horizontal), `background-position` uniforme e **sem** `transform: scale()` — caso contrário REJECT com motivo "panorama quebrado".
2. **Sobreposição sem sentido:** título atrás do número, texto sobre face da pessoa, dois elementos do mesmo peso sobrepostos, box duplicado no mesmo slide; **legenda de gráfico sobre o preenchimento da barra** (mesmo defeito de contraste); **barra horizontal cobrindo rótulo à esquerda** (rect começando antes da coluna de texto — REJECT).
